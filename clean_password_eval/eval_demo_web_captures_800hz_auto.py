...
def main() -> int:
    parser = argparse.ArgumentParser(description="Replay demo_web_test_800hz captures through the native 800Hz full-auto pipeline.")
    parser.add_argument("--dataset-root", default="data/raw/demo_web_test_800hz")
    parser.add_argument("--checkpoint-dir", default="demo_inference_api/inference/checkpoints_800hz_fullauto_stage12")
    parser.add_argument("--output-dir", default="results/800hz_stage12_baseline/demo_web_auto_eval_20260401")
    ...
    capture_paths = sorted(dataset_root.glob("*_capture.json"))
    if not capture_paths:
        raise SystemExit(f"No capture files found in {dataset_root}")
    ...
    for capture_path in capture_paths:
        ...
        truth = str(capture.get("typed_password", "")).strip()
        if not truth:
            continue

        sensor_csv = Path(capture["sensor_csv_path"])
        events_csv = Path(capture["events_csv_path"])
        ...
        if stage1_best is not None:
            ...
            hi = min(len(imu), int(stage1_best["end"]))
            seg = imu[lo:hi]
        ...
        flat_rows.append(
            {
                ...
                "pipeline_top_candidate": block.get("pipeline_top_candidate"),
                "pipeline_top100_hit": block.get("pipeline_top100_hit"),
                "ctc_prediction": block.get("ctc_prediction"),
                "ctc_cer": block.get("ctc_cer"),
