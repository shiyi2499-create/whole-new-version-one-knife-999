...
Run with:  python3 collector.py --mode single_key
           python3 collector.py --mode free_type
           (sudo is no longer required — direct SPU IOKit backend is the default)
"""
...
class DataCollector:
    ...
    def run(self):
        ...
        try:
            ...
            print("  Ensure this is an Apple Silicon Mac with BMI286 IMU.")
            print("  The direct SPU backend should work without root.")
            print("  If it fails, try: sudo .venv/bin/python3 collector.py (macimu fallback)")
...
def main():
    ...
    # Non-root is the default path (direct SPU IOKit backend).
    # Root is still supported for the legacy macimu fallback.
    is_root = hasattr(os, "geteuid") and os.geteuid() == 0
    print(f"  euid:        {os.geteuid() if hasattr(os, 'geteuid') else 'N/A'}")
    print(f"  Backend:     {'(will auto-detect)' if not is_root else '(root — macimu fallback available)'}\n")
