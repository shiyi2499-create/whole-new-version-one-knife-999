...
def _label_candidate(candidate: dict, ep, session_ts: np.ndarray) -> tuple[float, dict]:
    ...
    quality = float(0.7 * iou + 0.3 * key_frac)
    return quality, {"iou": float(iou), "key_frac": float(key_frac), "quality": quality}
...
def main():
    ...
    with open(out_dir / 'ranker_dataset_summary.json', 'w', encoding='utf-8') as f:
        json.dump({
            ...
            'quality_gt_05': int(np.sum(y>=0.5)),
            'quality_gt_025': int(np.sum(y>=0.25)),
            'quality_mean': float(np.mean(y)),
    ...
    for holdout in session_ids:
        ...
        val_pred = ranker.predict(X[val_mask])
        fold_reports.append({
            'holdout_session': holdout,
            'candidate_r2': float(r2_score(y[val_mask], val_pred)),
            'num_val_candidates': int(np.sum(val_mask)),
            'mean_quality_true': float(np.mean(y[val_mask])),
            'mean_quality_pred': float(np.mean(val_pred)),
        })
