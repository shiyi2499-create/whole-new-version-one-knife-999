...
def _search_best_k_sequence(peaks: np.ndarray, scores: np.ndarray, sample_rate: float, min_keys: int, max_keys: int, gap_prior_s: float):
    best_seq = None
    best_score = -1e18
...
def propose_energy_classifier_anchors(raw_imu: np.ndarray, sample_rate: float, classifier, device: torch.device, expected_keys: int = 8, min_keys: int = 6, max_keys: int = 12, gap_prior_s: float = 1.5):
    ...
    if expected_keys > 0:
        ...
        chosen, search_score, used_expected = _search_best_k_sequence(peaks, scores, sample_rate, min_keys=min_keys, max_keys=min(max_keys, len(peaks)), gap_prior_s=gap_prior_s)
...
def main():
    ...
    for session_path in sorted({ep.session_path for ep in episodes}):
        ...
        local_anchor_frames, anchor_debug = propose_energy_classifier_anchors(crop_imu, effective_sr, overlap_model.classifier, device, args.expected_keys, args.min_keys, args.max_keys, args.gap_prior_s)
        ...
        for ep in session_eps:
            if len(pred_global_ts) == 0:
                debug_rows.append({"episode_id": ep.episode_id, "session_id": session_id, "coarse_region": {"start_s": region.start_s, "end_s": region.end_s}, "anchor_debug": anchor_debug, "error": "no_anchors"})
                continue
