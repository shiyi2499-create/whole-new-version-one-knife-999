"""
Signal processing utilities (shared logic, no torch dependency).
Identical to stage2_open version — kept as a local copy for independence.
"""
import numpy as np
...
def preprocess(data: np.ndarray, sample_rate=100, add_mag=True, norm=True,
               stats=None):
    """
    Full preprocess: magnitude → normalize.
    Returns (processed, stats_dict).
    """
...
def splice_smooth(a: np.ndarray, b: np.ndarray, overlap: int = 10) -> np.ndarray:
    """Overlap-add two signals [T_i, C]."""
    if overlap <= 0 or overlap > min(len(a), len(b)):
        return np.concatenate([a, b], axis=0)
    fade_out = np.linspace(1, 0, overlap)[:, None]
    fade_in = np.linspace(0, 1, overlap)[:, None]
    blended = a[-overlap:] * fade_out + b[:overlap] * fade_in
    return np.concatenate([a[:-overlap], blended, b[overlap:]], axis=0)


def time_stretch(data: np.ndarray, rate: float) -> np.ndarray:
    """rate > 1 = speed up (shorter)."""
    T, C = data.shape
